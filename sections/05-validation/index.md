...
* **Dual-Tier Binary Telemetry Integration:** test_framing.py imports identical byte sequences emitted by microshield_cobs.c, confirming that field offsets, endianness conversions, float IEEE 754 representations, and CRC32 calculations match across C and Python with 100% success.
* **AST Transpilation & Compiler Loop Integration:** The transpiler generates transpiled_model.h, which is included directly into microshield_engine.c and compiled under -std=c99 -Wall -Wextra -Werror with zero warnings.
...
3. **Statistical Drift Injection:** Injects borderline timing variations (delta_t between 70 us and 95 us) and payload entropy drift. The edge engine emits VERDICT_AMBIGUOUS records.
4. **Drift Alarm Evaluation:** The supervisory sliding window ingests ambiguous verdicts. Once the ambiguity ratio surpasses the 5.0% threshold, the UI triggers the DRIFT DETECTED visual alert.
...
* **Visual Status Indicators:**
  * **Green Indicator (PB0):** Nominal operation (VERDICT_BENIGN), packet forwarded to process queue.
  * **Orange Indicator (PB1):** Borderline telemetry (VERDICT_AMBIGUOUS), packet quarantined, drift counter updated.
  * **Red Indicator (PB2):** Malicious intrusion (VERDICT_ATTACK), payload dropped, alert telemetry dispatched.
  * **User Control (PC0):** Pushbutton with internal pull-up and 100 nF hardware anti-bounce capacitor for manual reset.
...
* Each counter tick corresponds to: 1 tick = 1 / 168 MHz = 5.952 nanoseconds.
* Execution latency is calculated by reading the counter register immediately before and after the critical code section:
  Execution Time (us) = (CYCCNT_stop - CYCCNT_start) / 168.0

Because register sampling requires a single-cycle assembly read instruction (LDR), the measurement introduces negligible overhead (under 12 nanoseconds), providing cycle-exact execution figures without external instruments.
...
The NUCLEO-F407RE exposes a dedicated power isolation bridge via jumper JP6 (labeled IDD). Removing this jumper isolates the STM32F407RE microcontroller VDD rail (3.3V) from the rest of the board (ST-Link programmer, USB LDO, and debug circuits).
...
| Operating Phase | MCU Voltage (VDD) | Measured Current (IDD) | Active Duration (t) | Energy per Event (E) |
| :--- | :--- | :--- | :--- | :--- |
...
* Total active CPU duty cycle: 100 * 31.50 us = 3.15 ms/second (only 0.315% active duty cycle).
