...
1. **Ingress & Feature Calculation (`microshield_features.c`):** Inbound network frames residing in DMA receive buffers are processed via zero-copy pointer access, extracting the 16-byte `microshield_features_t` structure in bounded execution time (&le; 18 &mu;s).
...
3. **Framing & Serial Egress (`microshield_cobs.c`):** Frames classified as `ATTACK` or `AMBIGUOUS` trigger the construction of a 32-byte packed `microshield_telemetry_t` frame, protected by an IEEE 802.3 CRC32 checksum and encoded using Consistent Overhead Byte Stuffing (COBS).
...
### 4.1.4 Quality Assurance & Strict Compilation Diagnostics
...
#### Dedicated C99 Compiler Flag Specification
During host verification and edge library compilation, GCC is invoked with an uncompromising set of diagnostic flags:
* **`-std=c99`:** Mandates strict adherence to the ISO/IEC 9899:1999 standard. Disabling proprietary GNU extensions (`gnu99`) guarantees absolute cross-compiler portability between the desktop testing environment (`gcc`) and the embedded ARM cross-toolchain (`arm-none-eabi-gcc`).
* **`-Wall -Wextra`:** Enables the full baseline and extended diagnostic suites, catching uninitialized variables, sign-comparison mismatches, and implicit conversions before runtime execution.
* **`-Werror`:** Enforces a zero-tolerance defect policy by promoting every single compiler warning to a fatal, build-terminating error. This prevents developers from ignoring subtle pointer alignments or buffer boundary notices.
* **`-Iinclude -Imodel`:** Enforces clean, decoupled preprocessor include paths, isolating public header contracts (`microshield.h`) from private static decision tables (`transpiled_model.h`).
* **`-lm`:** Explicitly links the standard mathematical runtime library (`libm`), required for single-precision floating-point operations such as `fabsf()` during feature validation.

#### Static Type Theory in Python (PEP 484 & PEP 526)
In the supervisory tier, code quality is governed by static type theory rather than dynamic type inference. Leveraging modern Python type specifications, the MLOps pipeline enforces strict static typing via `mypy` configured in full strict mode (`--strict`). This configuration systematically prohibits dynamically typed functions, untyped decorators, and ambiguous return values, ensuring that domain entities and value objects remain strictly typed, immutable, and provably correct before execution.

#### Test Design Methodology & AI-Assisted Scaffolding
In accordance with modern software engineering practices, test case design was structured around rigorous human specification:
1. **Human Engineering Domain:** The human architect defined the test matrices, boundary value conditions (such as the 32-bit hardware timer rollover from `0xFFFFFFF0` to `0x00000010` and runt frames under 14 bytes), mathematical oracles (IEEE 802.3 CRC32 constant `0xCBF43926`), and drift threshold rules (5% ambiguity ceiling).
2. **AI-Assisted Syntactic Scaffolding:** Automated language models were deployed strictly as syntactic scaffolding aids to generate repetitive harness boilerplate and parameterized stimulus vectors conforming to the human test matrix.
3. **Supervised Acceptance Gate:** Every generated test was independently reviewed, enriched with explicit requirement docstrings, and validated through compilation with `-Werror` and execution under `mypy --strict`.
...
2. **Feature Extraction Call (`microshield_features.c`):** The callback invokes `microshield_extract_features()`. The module calculates the 16-byte `microshield_features_t` in bounded execution time (&le; 18 &mu;s) using the hardware DWT cycle counter for microsecond-level timing delta acquisition.
3. **Deterministic Traversal Call (`microshield_engine.c`):** The feature vector is passed to `microshield_classify()`. The engine traverses the static binary decision tree defined in `transpiled_model.h` in bounded time (&le; 12 &mu;s), resolving the ternary verdict (`BENIGN`, `ATTACK`, `AMBIGUOUS`), the leaf `rule_id`, and the dominant `split_feature`.
...
<span style="color: #0284c7;">int</span> <span style="color: #4338ca; font-weight: bold;">main</span>(<span style="color: #0284c7;">void</span>) {
    ...
    <span style="color: #0284c7;">while</span> (1) {
        <span style="color: #94a3b8;">/* Race-to-Sleep: halt core clock until a peripheral interrupt arrives */</span>
        __WFI();
    }
}
...
MicroShield establishes a **Transport-Agnostic Zero-Trust Boundary**: diagnostic records are algebraically sealed directly in STM32 silicon and validated exclusively upon domain deserialization within the Python supervisory runtime. The underlying physical carrier remains completely transparent to the telemetry contract.
...
### 4.4.3 Verification Evidence & Edge Unit Test Results
...
## 4.7 Reactive Operator Dashboard & Empirical Monorepo Verification
...
Developed using Plotly Dash and Flask, the web console enforces a strict unidirectional information hierarchy:
...
2. **KPI Card Visualizations:** The top grid summarizes core health indicators of monitored edge nodes (Ambiguity Ratio, Total Processed, Drift State badge switching between `NOMINAL` and `DRIFT DETECTED`).
...
4. **Human-in-the-Loop MLOps Retraining:** When drift occurs, the operator is presented with the **Trigger AST Retraining** action. Activating this control triggers the transpiler, re-fits the decision tree against the latest ambiguous samples, resets the drift detector, and emits an updated `transpiled_model.h`.

### 4.7.2 UI Unit Test Matrix
...
| `UT-UI-03` | `test_ui.py` | Callback Map Integrity | Reactive callbacks registered in the app context | **PASSED** |
| `UT-UI-04` | `test_ui.py` | Figure Builders | Plotly graph figures successfully synthesized | **PASSED** |
| `UT-UI-05` | `test_ui.py` | Drift State UI Reaction | Ingestion of drifted detector triggers `DRIFT DETECTED` | **PASSED** |

### 4.7.3 Unified Monorepo Execution Verification (`make test`)

To provide empirical evidence of system integrity, the entire regression suite spanning both languages is executed through the unified root `Makefile`. The actual command-line execution log confirms that all 36 unit tests pass and all 20 source files satisfy strict static type safety:
...
=== [1/3] Running Bare-Metal C99 Edge Test Suite ===
make -C edge test
...
rootdir: /home/wearemassive/microshield/artifact/supervisor
plugins: dash-2.18.2
collected 24 items
...
tests/test_drift.py::test_concept_drift_alarm_trigger PASSED              [ 12%]
tests/test_drift.py::test_fifo_sliding_window_recovery PASSED             [ 16%]
tests/test_drift.py::test_detector_reset PASSED                           [ 20%]
tests/test_drift.py::test_invalid_parameters_raise_value_error PASSED     [ 25%]
tests/test_framing.py::test_decode_valid_frame_matching_c_test PASSED     [ 29%]
tests/test_framing.py::test_corrupted_crc_rejection PASSED                [ 33%]
tests/test_framing.py::test_truncated_frame_rejection PASSED              [ 37%]
tests/test_trainer.py::test_dataset_generation_dimensions PASSED          [ 41%]
tests/test_trainer.py::test_tree_depth_hardware_constraint PASSED         [ 45%]
tests/test_trainer.py::test_tree_classification_performance PASSED        [ 50%]
tests/test_transpiler.py::test_tree_depth_exceeded_guard PASSED           [ 54%]
tests/test_transpiler.py::test_c_header_generation_syntax PASSED          [ 58%]
tests/test_transpiler.py::test_rule_dictionary_xai_generation PASSED      [ 62%]
tests/test_transpiler.py::test_artifacts_disk_emission PASSED              [ 66%]
tests/test_types.py::test_verdict_numeric_values PASSED                   [ 70%]
tests/test_types.py::test_feature_vector_immutability PASSED              [ 75%]
tests/test_types.py::test_telemetry_record_structure_and_integrity PASSED  [ 79%]
tests/test_ui.py::test_dashboard_instantiation PASSED                     [ 83%]
tests/test_ui.py::test_layout_contains_critical_components PASSED         [ 87%]
tests/test_ui.py::test_callback_registration PASSED                       [ 91%]
tests/test_ui.py::test_figure_builders PASSED                             [ 95%]
